...
def brute_unzip(file = None, p = 0, resume = None):
    data = {}
    ...
    p_start = max(p,data['password number'])
    p = p_start

    pw = None
    list_pw = []
    start_sec = time.mktime(datetime.now().timetuple())
    data['start sec'] = start_sec
    data['start datetime'] = str(datetime.now())
    ...
    print('--- --- ---', file, '|', data['start datetime'], '--- --- ---')
    ...
    while True:
        ...
        if zipper.unzip_file():
            ...
            data['report'] = zipper.report()
            data['sec/n'] = self.perf['sec/n']
            data['n/sec'] = self.perf['n/sec']
            data['directory'] = zipper.zipDir
            data['seconds run'] = zipper.secs
            data['number of files'] = zipper.progress
            data['current sec'] = zipper.current
            data['current datetime'] = str(datetime.now())
            ...
            # if randint(1,1000) == 1:
                ...
                if randint(1,5) == 1:
                    ...
                    data['password'] = zipper.password
                    data['report'] = zipper.report()
                    data['sec/n'] = zipper.perf['sec/n']
                    data['n/sec'] = zipper.perf['n/sec']
                    # data['directory'] = zipper.zipDir
                    data['seconds run'] = zipper.secs
                    data['number of files'] = zipper.progress
                    data['current sec'] = zipper.current
...
zip_file = 'C:/Users/Konrad/Desktop/RPi3/DEÜV_Meldung_2018_01_917400_Container.ZIP'
# password = 'password'
